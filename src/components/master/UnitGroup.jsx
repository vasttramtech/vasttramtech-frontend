...
const UnitGroup = ({ setUnitModel ,setRefresh,refresh }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(
                `${process.env.REACT_APP_BACKEND_URL}/api/units`,
                payload,
                {
                    headers: {
            ...
            setUnit("");
            toast.success("Unit saved successfully!", { position: "top-right" });
            setUnitModel(false);
        } catch (error) {
    ...
    return (
        <div className="bg-white p-6 w-[400px] rounded-lg shadow-lg border border-gray-300">
            <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">Add Unit Group</h2>

            {/* Form */}
            <form onSubmit={handleSubmit}>
                {/* Design Group */}
                <div className="mb-4">
                    <label className="text-gray-700 font-semibold block mb-1">Unit Group</label>
                    <input
                        type="text"
