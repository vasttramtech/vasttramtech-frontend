...
const ExtraBOMSfg = ({
  ...
  const fetchSalesOrder = async () => {
    try {
      ...
      if (!response || !response.data) {
        ...
        const considereddata = response.data?.extra_bom_so[0]?.Extra_bom[-1];
        const transformedData = {
          semi_finished_goods:
            considereddata.semi_finished_goods?.semi_finished_goods_name || "",
          qty: Number(considereddata.qty),
          total_price: Number(considereddata.total_price),
          sfg_description: considereddata.sfg_description || "",
          color: considereddata?.color?.color_name,
          unit: considereddata?.unit?.unit_name,
          raw_material_bom: (considereddata.raw_material_bom || []).map(
            (rm) => ({
          ...
          jobber_master_sfg: (considereddata.jobber_master_sfg || []).map(
            (jobber) => ({
          ...
          color: considereddata.color,
          bom_status: considereddata?.bom_status,
          new_sfg: false,
          stock_status: considereddata?.stock_status,
        };
        setAllSavedSemiFinishedGoods(transformedData);
        const savedData = {
          semi_finished_goods: considereddata?.semi_finished_goods?.id,
          qty: Number(considereddata?.qty),
          color: considereddata?.color?.id,
          processes: considereddata?.processes.map((item) => ({
          ...
          total_price: Number(considereddata?.total_price),
          sfg_description: considereddata?.sfg_description,
          raw_material_bom: considereddata?.raw_material_bom.map((rm) => ({
        ...
        setsfglist((prev) => [...prev, considereddata?.semi_finished_goods]);
      }
    } catch (error) {
  ...
  const handleAddSelectedSFG = async (e) => {
    ...
    try {
      // setAllSemiFinishedGoods((prev) => [...prev, ...postData.data.extra_bomSfg_fromStock]);
      // setsfglist((prev) => [...prev, finalselectedSFG]);
      ...
      await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}/api/${api_point}/${so_id}/extra-bom-from-stock`,
        postData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.success("Extra Bom from stock added successfully", {
        position: "top-right",
      });
      ...
      //     if (res.status === 200) {
      //       type === "internal-sales-order-entries"
      //         ? (type = "internal-sales-order-entry")
      //         : (type = "sales-oder-entries");
      //       try {
      //         setLoading(true);

      //         const { data } = await axios.get(
      //           `${process.env.REACT_APP_BACKEND_URL}/api/${type}/find-by-id/${id}`,
      //           {
      //             headers: {
      //               Authorization: `Bearer ${token}`,
      //             },
